#!/bin/bash
wget -O SMC.jar "https://docs.google.com/uc?id=0B5_wYgbEk-GZOUY5X2lHVURTLUk&export=download"

chmod a+x SMC.jar

perl gdown.pl 'https://docs.google.com/uc?id=0B5_wYgbEk-GZZURtbnJIeUFEU2M&export=download' 'mac-mc.zip'

#https://drive.google.com/file/d/0B5_wYgbEk-GZZURtbnJIeUFEU2M/view?usp=sharing

#https://docs.google.com/uc?id=0B5_wYgbEk-GZNmxfRmdmeWxUelE&export=download

#https://docs.google.com/uc?export=download&confirm=R9EE&id=0B5_wYgbEk-GZNmxfRmdmeWxUelE

#https://docs.google.com/uc?id=0B5_wYgbEk-GZZURtbnJIeUFEU2M&export=download




